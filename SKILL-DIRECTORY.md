# AI Marketing System — Skill Directory
*Последна проверка: 10.05.2026*

---

## Общ статус: 30 skill folders | 30 с SKILL.md | 9 празни папки

---

## ✅ АКТИВНИ SKILLS (имат SKILL.md)

### 🔵 SYSTEM
| Skill | Описание | Последна промяна |
|-------|----------|-----------------|
| `start` | Входна точка — зарежда Brand DNA, показва меню, рутира | Оригинален |
| `brand-dna` | 15-частна структура, Drive-first extraction, 6 стъпки | ✅ ОБНОВЕН (10.05.2026) |
| `quality-check` | Frank Kern оценка 0-10, итерира до 8+. Работи за всичко | Оригинален |

### 🟢 CHALLENGE CHAIN
| Skill | Описание | Последна промяна |
|-------|----------|-----------------|
| `challenge-concept` | Challenge name + Belief Ladder + 3-day arc + VIP + bump | ✅ Belief Ladder добавен |
| `challenge-landing` | **ПЪЛНА 6-PAGE FUNNEL** — free landing + VIP + checkout + 3 TY pages | ✅ ОБНОВЕН (10.05.2026) |
| `challenge-posts` | 5-8 promo posts + 20+ hooks за регистрации | Оригинален |
| `challenge-ads` | Ad copy + Gemini visual prompts + video scripts | Оригинален |
| `challenge-emails` | Email sequences: регистрирани + не-регистрирани + VIP | ⚠️ Непълен (липсват bump + scarcity + reminder имейли) |
| `challenge-days` | Дневни описания — coach guide за всеки ден | Оригинален |
| `challenge-script` | Offer close script за финалния ден | Placeholder |

**Challenge Chain Flow:**
```
/challenge-concept → /challenge-landing (6 pages) → /challenge-posts → /challenge-ads → /challenge-emails → /challenge-days → /challenge-script
```

### 🟣 WEBINAR CHAIN
| Skill | Описание | Последна промяна |
|-------|----------|-----------------|
| `webinar-concept` | Тема + teaching points + offer момент | Оригинален |
| `webinar-script` | Пълен скрипт (Brunson + Kern + Hormozi + Fletcher) | Оригинален |
| `webinar-landing` | **Landing + Thank You page** — file index + section-by-section + Frank Kern score | ✅ ОБНОВЕН (10.05.2026) |
| `webinar-ads` | Ad copy + Gemini prompts + video scripts | Оригинален |
| `webinar-emails` | Pre-webinar (4) + post-attended (3) + post-no-show (3) | Оригинален |

**Webinar Chain Flow:**
```
/webinar-concept → /webinar-script → /webinar-landing → /webinar-ads → /webinar-emails
```

### 🟡 LEAD MAGNET CHAIN
| Skill | Описание | Последна промяна |
|-------|----------|-----------------|
| `lead-magnet-concept` | Desire-first — какво искат толкова силно, че дават имейл | Оригинален |
| `lead-magnet-content` | Съдържание на lead magnet-а (ready for Canva/Notion) | Оригинален |
| `lead-magnet-landing` | Opt-in page + thank you page | Оригинален |
| `lead-magnet-posts` | 3-5 promo posts — pain/story/direct angles | Оригинален |
| `lead-magnet-ads` | Ad copy + Gemini prompts + video scripts | Оригинален |
| `lead-magnet-emails` | Welcome + 4-email nurture sequence | Оригинален |

**Lead Magnet Chain Flow:**
```
/lead-magnet-concept → /lead-magnet-content → /lead-magnet-landing → /lead-magnet-posts → /lead-magnet-ads → /lead-magnet-emails
```

### 🟠 STANDALONE TOOLS
| Skill | Описание | Последна промяна |
|-------|----------|-----------------|
| `ads` | Ads за всякаква оферта (не е campaign-specific) | Оригинален |
| `offer` | Изгражда/подобрява high-ticket оферта | Оригинален |
| `cover-photo` | Gemini prompt за Facebook group cover (940x348px, 3-zone) | Оригинален |
| `content-calendar` | 16 поста за месец по Frank Kern framework | Оригинален |

### 🔴 POST SKILLS
| Skill | Описание | Последна промяна |
|-------|----------|-----------------|
| `post-hooks` | 20+ hook идеи от Brand DNA | Оригинален |
| `post-belief` | Belief-shifting long-form (5 формули) | Оригинален |
| `post-offer` | Facebook offer post с Magnetic Message | Оригинален |
| `post-social-proof` | Before/after, case study, result compilations | Оригинален |
| `post-viral` | Two-step viral posts — comment-generating | Оригинален |
| `post-carousel` | Трансформира пост в carousel prompts за Gemini | Оригинален |

---

## ❌ ПРАЗНИ ПАПКИ (folder exists, NO SKILL.md)

| Папка | Вероятно предназначение | Приоритет |
|-------|------------------------|-----------|
| `challenge-builder` | Orchestrator за целия challenge chain | 🟡 Medium |
| `challenge-vip` | Остатък от стара структура — superseded от challenge-landing | 🔴 Delete or archive |
| `email-sales` | Sales email (за след offer/sales call) | 🟡 Medium |
| `email-sequence` | Generic имейл последователности | 🟡 Medium |
| `email-singles` | Single promotional emails | 🟢 Low |
| `email-webinar` | Имейли за webinar (duplicate на webinar-emails?) | 🔴 Check if duplicate |
| `email-welcome` | Welcome sequence за нови абонати | 🟡 Medium |
| `landing-page` | Generic landing page (не campaign-specific) | 🟡 Medium |
| `lead-magnet` | Вероятно orchestrator — lead-magnet-concept го замества | 🔴 Check if needed |
| `posts` | Generic posts skill — post-* skills го заместват? | 🔴 Check if needed |

---

## 🔧 KNOWN GAPS (от SKILL-GAP-ANALYSIS.md)

| Gap | Статус | Бележка |
|-----|--------|---------|
| challenge-emails — bump offer имейли (3-5) | ❌ Липсва | Нужно |
| challenge-emails — scarcity имейли (2) | ❌ Липсва | Нужно |
| challenge-emails — reminder имейли (4, по 1 на ден) | ❌ Липсва | Нужно |
| challenge-ads — enforce 6-8 angles минимум | ⚠️ Не е enforced | Minor |
| quality-check — challenge-specific checklist | ⚠️ Генеричен | Minor |

---

## 📁 PROMPT LIBRARY (05-Prompts/)

| Prompt | Файл | Интегриран в |
|--------|------|-------------|
| Webinar Page Builder V1 | `05-Prompts/06-webinar/05-webinar-landing-page-builder.md` | `webinar-landing/SKILL.md` |
| Challenge Funnel Builder V1 (complete) | `05-Prompts/07-challenge/01-challenge-funnel-builder-complete.md` | `challenge-landing/SKILL.md` |
| Challenge VIP Funnel Builder V1 (VIP-only reference) | `05-Prompts/07-challenge/01-challenge-vip-funnel-builder.md` | Superseded — reference only |

---

## 🗺️ COMPLETE SKILL MAP

```
/start
│
├── CHALLENGE CHAIN
│   ├── /challenge-concept      ← Belief Ladder + 3-day arc
│   ├── /challenge-landing      ← 6-page funnel (free + VIP + checkout + 3 TY)
│   ├── /challenge-posts        ← 5-8 promo posts
│   ├── /challenge-ads          ← ad copy + visuals + video
│   ├── /challenge-emails       ← ⚠️ incomplete (missing bump/scarcity/reminders)
│   ├── /challenge-days         ← daily coach guide
│   └── /challenge-script       ← offer close script (placeholder)
│
├── WEBINAR CHAIN
│   ├── /webinar-concept        ← topic + teaching + offer moment
│   ├── /webinar-script         ← full presentation script
│   ├── /webinar-landing        ← landing + TY page (file index system)
│   ├── /webinar-ads            ← ad copy + visuals + video
│   └── /webinar-emails         ← pre + post sequences
│
├── LEAD MAGNET CHAIN
│   ├── /lead-magnet-concept    ← desire-first concept
│   ├── /lead-magnet-content    ← actual content
│   ├── /lead-magnet-landing    ← opt-in + TY page
│   ├── /lead-magnet-posts      ← 3-5 promo posts
│   ├── /lead-magnet-ads        ← ad copy + visuals
│   └── /lead-magnet-emails     ← welcome + 4 nurture
│
├── STANDALONE TOOLS
│   ├── /brand-dna              ← 15-part structure (Drive-first)
│   ├── /offer                  ← high-ticket offer builder
│   ├── /ads                    ← standalone ads
│   ├── /cover-photo            ← Facebook group cover
│   ├── /content-calendar       ← 16 posts/month
│   └── /quality-check          ← Frank Kern 0-10 scorer
│
└── POST SKILLS
    ├── /post-hooks             ← 20+ hook ideas
    ├── /post-belief            ← belief-shifting posts
    ├── /post-offer             ← offer posts
    ├── /post-social-proof      ← social proof posts
    ├── /post-viral             ← two-step viral posts
    └── /post-carousel          ← carousel prompts for Gemini
```

---

## ✅ RECENTLY UPDATED (10.05.2026)

1. **`brand-dna/SKILL.md`** — Drive-first, 15-part structure, targets webinar/sales call transcripts
2. **`challenge-landing/SKILL.md`** — Complete 6-page funnel (was just a landing page stub)
3. **`webinar-landing/SKILL.md`** — File index system, hard-coded standards, section-by-section building
4. **`SKILL-GAP-ANALYSIS.md`** — Updated: 10/11 phases covered (was 8/11)
5. **`05-Prompts/06-webinar/05-webinar-landing-page-builder.md`** — Raw prompt saved
6. **`05-Prompts/07-challenge/01-challenge-funnel-builder-complete.md`** — Raw prompt saved

---

*Generated: 10.05.2026*
